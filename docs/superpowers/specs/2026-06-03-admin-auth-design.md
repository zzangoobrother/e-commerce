...
> **최신 보안 한계 현황은 [README의 "보안 한계" 섹션](../../../../README.md#보안-한계-골격-수준--운영-전-보완-필요)을 단일 출처로 한다.**
> 아래 목록은 어드민 인증 골격 시점의 한계이며, 일부는 2026-06-04 보안 보완 사이클로 해소됨.

- **[해소됨 — 2026-06-04]** ~~일반 쿠키 저장 (httpOnly 아님) → XSS 취약~~ → httpOnly 쿠키(+SameSite=Lax) 전환 완료
- **[해소됨 — 2026-06-04]** ~~로그인 시도 제한(brute-force 방어) 없음~~ → IP 기준 5회/15분 제한 추가
- 리프레시 토큰 없음 → 1시간마다 재로그인 (남은 한계 — 다음 사이클)
- 토큰 서버 측 무효화 불가 (로그아웃은 클라이언트 쿠키 삭제만) (남은 한계 — 다음 사이클)

...
> 마지막 갱신: 2026-06-06 (보안 보완 사이클 문서 단일 출처화 완료 시점)
...
### 후보 1: 보안 보완 2차 — 토큰 수명 주기 (추천)

2026-06-04 보안 보완 사이클로 httpOnly 쿠키 전환·시도 제한·타이밍 완화 등 핵심 8건이 해소됐다.
남은 한계 목록은 README "보안 한계 > 남은 한계" 섹션을 단일 출처로 한다.

| 항목 | 근거 | 규모 |
|------|------|------|
| 리프레시 토큰 도입 | 현재 1시간마다 재로그인 필요 | 중 |
| 토큰 서버 측 무효화 (블랙리스트) | 로그아웃 후에도 만료 전까지 토큰 유효 | 중 |

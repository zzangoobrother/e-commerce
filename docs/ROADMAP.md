...
> 마지막 갱신: 2026-06-17 (배송 상태·어드민 주문 관리 사이클)
...
주문은 현재 결제 없이 즉시 확정(ORDERED)된다. 남은 범위:

- **결제 연동(PG)**: 결제 게이트웨이 연동, 결제 상태 관리(결제 대기/완료/실패), 주문과 결제 상태의 정합성.
- **반품·교환**: 배송완료 이후의 반품·교환 흐름(현재 취소는 ORDERED 단계만).

...
> 마지막 갱신: 2026-06-12 (주문 사이클 문서 동기화)
...
### 후보 5: 주문 후속 — 결제 · 배송 · 어드민 주문 관리

사이클 10에서 주문 도메인의 핵심(생성·부분 주문·취소·재고)이 완료됐다. 남은 범위:

- **결제**: 현재는 결제 없이 주문 즉시 확정(ORDERED). 결제 연동·결제 상태 관리.
- **배송 상태**: 주문 상태가 ORDERED/CANCELLED 단일 전이뿐 — 배송 진행 상태(준비/배송중/완료) 도입.
- **어드민 주문 관리**: 어드민이 전체 주문을 조회·상태 변경할 수 있는 화면/API (현재 주문 API는 고객 전용).

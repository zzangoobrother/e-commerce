...
| 14. 배송지 관리(주소록 CRUD) | 2026-07-08 | 신규 `CustomerAddress`(스칼라 customerId, Order 패턴), 등록·조회(기본 먼저·최신순)·수정(기본여부 불변)·삭제(기본 삭제 시 최신 자동 승격)·기본지정(기존 자동 해제), 기본배송지 항상 0/1개 불변식·소유권 404·상한 10개, 고객 전용 인가, 프론트 `/addresses` 페이지·Server Action | `feature/address-book` 브랜치 (머지 대기) |
...
- **배송지/청구 주소**: 배송지 주소록 CRUD는 사이클 14(2026-07-08)로 완료. 남은 범위는 주문·결제 생성 시 배송지 선택 연동과 청구 주소 관리(후속 사이클).
